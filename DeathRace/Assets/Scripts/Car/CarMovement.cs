...
public class CarMovement : MonoBehaviour
{
  ...
  private float m_jumpButtonValue;
  [Space]
  ...
  private WaitForSeconds m_jumpWait;
  ...
  public float m_RollBackDeceleration;
  ...
  private void Start()
  {
    ...
    m_jumpWait = new WaitForSeconds(m_JumpDelay);
  ...
  private void Update()
  {
    ...
    m_jumpButtonValue = Input.GetAxis(m_jumpButton);

    if(m_jumpButtonValue > 0 && !m_Jumped)
    {
      Jump();
    }
  ...
  private void Move()
  {
    ...
    else if (m_movementInputValue < 0 && m_CurrentSpeed <= 0 && m_Stopped == false)
    {
      AccelerateBackward();
    }
  ...
  private void coast()
  {
    ...
    if (m_CurrentSpeed < (0 - m_DeadZone) || m_CurrentSpeed > (m_DeadZone))
    {
      if (m_CurrentSpeed < 0)
      {
        ...
        m_CurrentSpeed = m_CurrentSpeed - (m_CurrentSpeed * m_RollBackDeceleration) * Time.deltaTime;
      }
  ...
  private void Turn()
  {
    ...
    else if (m_CurrentSpeed >= 0 && m_CurrentSpeed < m_VCrit)     //For turning wheels while standing still.
    {
      m_turnValue = m_turnValue * m_turnInputValue;

      
      m_LeftWheel.localRotation = Quaternion.Euler(0, 0, m_turnInputValue * 40);
      m_RightWheel.localRotation = Quaternion.Euler(0, 0, m_turnInputValue * 40);
